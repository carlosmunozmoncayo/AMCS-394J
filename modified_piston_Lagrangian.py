...
def setup(use_petsc=False,outdir='./_output',solver_type='sharpclaw',
          ...
          absorbing_BC=False, start_absorbing=20*np.pi, stop_absorbing=30*np.pi):#tfluct_solver=True):
    ...
    #Setting initial constant state
    V0 = 1.
    u0 = 0.
    p0 = 1/gamma
    ...
    def b4step_burger(solver,state):
        ...
        state.q[0,:] = np.where(x_switch<0.5,V,V0)
        state.q[2,:] = np.where(x_switch<0.5,eps,eps0)
    ...
    #"""Initial pulse generated at left boundary by prescribed motion"""
        if dim.on_lower_boundary:
            ...
            #Computing tilde quantities on first cell
            #V1+= V
            #u1+= u
            #eps1+= eps
            p1 = (gamma-1.)*((eps1+eps)-0.5*u1**2)/(V1+V)

            #Computing tilde quantities on ghost cell
            p0 = p1 - M*np.cos(t)*deltaxi
            u0 = -2*M*np.sin(t) - u1
            deltap = (p1-p0)/(p1+p0)
            V0 = -V + (V1+V)*(gamma +deltap)/(gamma-deltap)
            eps0 = -eps + 0.5*u0**2 + p0*(V0+V)/(gamma-1) 
    ...
    if euler_RS == 'euler_burger':
        ...
        solver.before_step = b4step_burger
